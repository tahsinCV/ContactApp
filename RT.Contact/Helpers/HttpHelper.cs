...
namespace RT.Contacts.Helpers
{
    public class HttpHelper<T> : IHttpHelper<T>
    {
        ...
        {
            T result = default(T);
            var response = await Client.PostAsync(apiUrl, JsonContent.Create(postObject), cancellationToken).ConfigureAwait(false);
            if (response.IsSuccessStatusCode)
            {
                await response.Content.ReadAsStringAsync().ContinueWith((Task<string> x) =>
                {
        ...
        {
            var response = await Client.PutAsync(apiUrl, JsonContent.Create(putObject), cancellationToken).ConfigureAwait(false);
            if (!response.IsSuccessStatusCode)
            {
                var content = await response.Content.ReadAsStringAsync();
                response.Content?.Dispose();
